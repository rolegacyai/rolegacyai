# RolelegacyAI — Website

> **The AI memory layer that stays when people leave.**

This is the public-facing website for [RolelegacyAI](https://rolelegacyai.com) — a role-scoped institutional memory platform that helps organisations preserve role knowledge, speed up onboarding, and compound institutional intelligence across successive role holders.

---

## Project overview

RolelegacyAI helps organisations answer a difficult question: *what does this role actually know?* Not what's in the wiki. Not what's in someone's head. What the role itself has accumulated — decisions, lessons, workarounds, operational patterns — from every person who has ever held it.
...
This site is configured for GitHub Pages via the `rolelegacyai/rolelegacyai.github.io` repository.

### Deploy steps

1. **Ensure your repository is named** `rolelegacyai.github.io` under the `rolelegacyai` organisation.
...
4. GitHub will automatically build and publish. The site will be live at `https://rolelegacyai.github.io` within 1–2 minutes.

### Custom domain

To use `rolelegacyai.com`:
...
1. Ensure DNS is configured before adding the CNAME file:
   ...
   - OR add a `CNAME` record pointing `www` to `rolelegacyai.github.io`
2. Only after DNS is propagated: keep the `CNAME` file in this repository (it contains `rolelegacyai.com`).
3. In GitHub Pages settings, enter `rolelegacyai.com` as the custom domain and enable HTTPS.
...
General: hello@rolelegacyai.com
