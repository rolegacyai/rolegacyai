...
// ─── LANGUAGE SWITCHER — URL-based routing ────────────────────────────────────
(function initLanguageSwitcher() {
  ...
  function getBasePath() {
    ...
    const m = path.match(/^\/(zh|hi|mr)(\/.*)?$/);
    if (m) return m[2] || '/';
    return path;
  }

  function buildUrl(targetLang) {
    const base = getBasePath().replace(/\.html$/, '') || '/';
    if (targetLang === 'en') {
      if (base === '/prototype') return '/prototype.html';
      return base === '/' ? '/' : base;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Localized pages already have static .lang-sw injected from HTML — skip.
    ...
    const LANGS = [
      ...
      { code: 'hi',  label: 'हिन्दी',  hreflang: 'hi' },
      { code: 'mr',  label: 'मराठी',   hreflang: 'mr' },
    ];
    ...
    div.innerHTML = LANGS.map(l => {
      ...
      return `<a href="${url}" class="lang-sw__btn${active ? ' lang-sw__btn--active' : ''}" hreflang="${l.hreflang}" aria-label="Switch to ${l.label}">${l.label}</a>`;
    }).join('');

    const menuToggle = document.querySelector('.nav__menu-toggle');
    navInner.insertBefore(div, menuToggle || null);

    if (!document.getElementById('rolegacy-lang-sw-style')) {
      const style = document.createElement('style');
      style.id = 'rolegacy-lang-sw-style';
      style.textContent = [
        '.lang-sw{display:inline-flex;align-items:center;gap:2px;padding:3px;border:1px solid rgba(228,229,228,0.12);border-radius:999px;background:rgba(15,15,16,0.88);backdrop-filter:blur(10px);flex-shrink:0;margin-left:0.5rem}',
        '.lang-sw__btn{border:0;border-radius:999px;padding:0.32rem 0.5rem;background:transparent;color:#a1a1aa;font-family:"Space Grotesk",system-ui,sans-serif;font-size:0.72rem;font-weight:700;cursor:pointer;transition:background .2s,color .2s;text-decoration:none;line-height:1;display:inline-block}',
        '.lang-sw__btn:hover{color:#f4f4f5}',
        '.lang-sw__btn--active{background:#d4a373;color:#0f0f10}',
        '@media(max-width:768px){.lang-sw{margin-left:0;margin-top:0.5rem}}',
      ].join('');
